...
async def create_underwriting_decision(
    ...
    log_audit(
        db, "underwriting_decision", decision.id, "create",
        user_id=current_user.id, user_role=current_user.role,
        scheme_id=scheme_id, entity_label=f"Member #{decision.member_id}",
        new_value=json.dumps({"decision": decision.decision, "status": decision.status}, default=str),
    )
    await db.commit()
...
async def update_decision(
    ...
    if data.notes is not None:
        old_values["notes"] = decision.notes
        new_values["notes"] = data.notes
        decision.notes = data.notes
    if data.override_reason is not None:
        old_values["override_reason"] = decision.override_reason
        new_values["override_reason"] = data.override_reason
        decision.override_reason = data.override_reason

    decision.modified_date = datetime.now(timezone.utc)
    decision.modified_user = current_user.id

    if new_values:
        log_audit(
            db, "underwriting_decision", decision.id, "update",
            user_id=current_user.id, user_role=current_user.role,
            scheme_id=decision.scheme_id, entity_label=f"Member #{decision.member_id}",
...
async def submit_questionnaire(
    ...
    log_audit(
        db, "enrollment_questionnaire", questionnaire.id, "create",
        user_id=current_user.id, user_role=current_user.role,
        scheme_id=scheme_id, entity_label=f"Member #{questionnaire.member_id}",
        new_value=json.dumps({"join_reason": questionnaire.join_reason}, default=str),
    )
    log_audit(
        db, "underwriting_decision", decision.id, "auto_create",
        user_id=current_user.id, user_role=current_user.role,
        scheme_id=scheme_id, entity_label=f"Member #{decision.member_id}",
        new_value=json.dumps({"decision": decision.decision, "status": decision.status}, default=str),
        reason="Auto-generated by enrollment engine",
    )
    await db.commit()
